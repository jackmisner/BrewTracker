...
def get_yeast_attenuation_analytics(ingredient_id):
    ...
    try:
        ...
        if analytics:
            ...
            return jsonify({"error": "Yeast ingredient not found or no analytics available"}), 404
...
def get_all_yeast_analytics():
    ...
    try:
        ...
        return jsonify({
            "yeast_analytics": analytics_list,
            "total_count": len(analytics_list)
        }), 200
...
@attenuation_analytics_bp.route("/yeast/<ingredient_id>/improved-estimate", methods=["GET"])
@jwt_required()
...
def get_attenuation_system_stats():
    ...
    try:
        ...
        yeast_with_data = Ingredient.objects(type="yeast", actual_attenuation_count__gt=0).count()
        
        # Get total data points collected
        total_data_points = 0
        high_confidence_yeast = 0
        
        for ingredient in Ingredient.objects(type="yeast", actual_attenuation_count__gt=0):
            total_data_points += ingredient.actual_attenuation_count
            if ingredient.attenuation_confidence and ingredient.attenuation_confidence >= 0.8:
        ...
        stats = {
            ...
            "data_coverage_percentage": round((yeast_with_data / total_yeast * 100), 1) if total_yeast > 0 else 0
        }
