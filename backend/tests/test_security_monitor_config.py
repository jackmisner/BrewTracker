...
class TestSecurityMonitorConfiguration:
    ...
    def test_default_thresholds(self):
        ...
        assert SecurityMonitor.get_failed_login_threshold() == SecurityMonitor.DEFAULT_FAILED_LOGIN_THRESHOLD
        assert SecurityMonitor.get_failed_login_window() == SecurityMonitor.DEFAULT_FAILED_LOGIN_WINDOW
        assert SecurityMonitor.get_suspicious_request_threshold() == SecurityMonitor.DEFAULT_SUSPICIOUS_REQUEST_THRESHOLD
        assert SecurityMonitor.get_suspicious_request_window() == SecurityMonitor.DEFAULT_SUSPICIOUS_REQUEST_WINDOW
    ...
    def test_flask_config_thresholds(self):
        ...
        with app.app_context():
            with patch.dict(os.environ, {
                ...
                assert SecurityMonitor.get_failed_login_threshold() == 15  # From app config
                assert SecurityMonitor.get_failed_login_window() == 2700   # From app config
    ...
    def test_is_ip_allowlisted_true(self, mock_extract_ip):
        ...
        mock_extract_ip.return_value = '127.0.0.1'  # Localhost should always be allowlisted
    ...
    @patch.dict(os.environ, {
        ...
        'SECURITY_FAILED_LOGIN_WINDOW': '60'
    })
...
class TestSecurityMiddlewareIntegration:
    ...
    def test_check_request_security_allowlisted_bypass(self):
        ...
        with app.test_request_context('/api/some-endpoint', environ_base={'REMOTE_ADDR': '127.0.0.1'}):
            with patch('utils.security_monitor.SecurityMonitor.is_ip_allowlisted', return_value=True) as mock_allowlisted:
    ...
    def test_check_request_security_blocked_ip(self):
        ...
        with app.test_request_context('/api/some-endpoint', environ_base={'REMOTE_ADDR': '203.0.113.200'}):
            with patch('utils.security_monitor.SecurityMonitor.is_ip_allowlisted', return_value=False):
                with patch('utils.security_monitor.SecurityMonitor.is_ip_blocked', return_value=True):
    ...
    def test_check_request_security_json_processing_uses_extracted_ip(self):
        ...
        with app.test_request_context('/api/some-endpoint', 
                                      environ_base={'REMOTE_ADDR': '203.0.113.300'},
                                      data='{"large_field":"' + 'x' * 1001 + '"}',
                                      content_type='application/json'):
            with patch('utils.security_monitor.SecurityMonitor.is_ip_allowlisted', return_value=False):
                with patch('utils.security_monitor.SecurityMonitor.is_ip_blocked', return_value=False):
                    with patch('utils.security_monitor.SecurityMonitor.log_suspicious_request') as mock_log:
                        check_request_security()
