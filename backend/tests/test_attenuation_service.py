...
from models.mongo_models import BrewSession, Ingredient, Recipe, RecipeIngredient, User, UserSettings
...
class TestAttenuationService:
    ...
    def test_calculate_actual_attenuation(self):
        ...
        assert AttenuationService.calculate_actual_attenuation(1.000, 1.010) is None  # Invalid OG
        assert AttenuationService.calculate_actual_attenuation(1.010, 1.050) is None  # FG > OG
    ...
    def test_update_yeast_attenuation_data(self, app):
        ...
        with app.app_context():
            ...
            success = AttenuationService.update_yeast_attenuation_data(str(yeast.id), 78.0)
            ...
            for attenuation in [76.0, 80.0, 77.0, 79.0]:
                AttenuationService.update_yeast_attenuation_data(str(yeast.id), attenuation)
    ...
    def test_get_improved_attenuation_estimate(self, app):
        ...
        with app.app_context():
            ...
            yeast1 = Ingredient(
                name="Theoretical Yeast",
                type="yeast",
                attenuation=75.0
            ).save()
            
            estimate = AttenuationService.get_improved_attenuation_estimate(str(yeast1.id))
            ...
            estimate = AttenuationService.get_improved_attenuation_estimate(str(yeast2.id))
