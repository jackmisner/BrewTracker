...
class DecisionNode(FlowchartNode):
    ...
    def execute(self, context: 'RecipeContext') -> NodeResult:
        ...
        if not condition_name:
            raise ValueError(f"DecisionNode {self.node_id} missing 'condition' in config")
...
class MultiDecisionNode(FlowchartNode):
    ...
    def execute(self, context: 'RecipeContext') -> NodeResult:
        ...
        if not conditions:
            raise ValueError(f"MultiDecisionNode {self.node_id} missing 'conditions' in config")
            ...
            if context.evaluate_condition(condition_name, condition_config):
                result = NodeResult(
                    ...
                    data={
                        ...
                        'conditions_evaluated': [c.get('condition') for c in conditions]
                    }
                )
                self.log_execution(context, result)
                return result
        ...
        if default_path:
            ...
            logger.error(f"MultiDecisionNode {self.node_id}: No conditions matched and no default path")
            result = NodeResult(
                next_node_id=None,
                stop_execution=True,
...
class ActionNode(FlowchartNode):
    ...
    def execute(self, context: 'RecipeContext') -> NodeResult:
        ...
        changes = context.execute_strategy(strategy_name, self.config.get('parameters', {}))
...
class ConditionalActionNode(FlowchartNode):
    ...
    def execute(self, context: 'RecipeContext') -> NodeResult:
        ...
        if not condition_name or not strategy_name:
            raise ValueError(f"ConditionalActionNode {self.node_id} missing 'condition' or 'strategy'")
