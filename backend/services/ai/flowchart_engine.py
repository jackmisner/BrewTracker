...
from .flowchart_nodes import FlowchartNode, create_node, NodeResult
...
class FlowchartEngine:
    ...
    def _build_nodes(self):
        ...
        logger.info(f"Built {len(self.nodes)} nodes for workflow: {self.workflow_config.get('workflow_name')}")
    
    def execute_workflow(self, recipe_data: Dict[str, Any], style_guidelines: Optional[Dict[str, Any]] = None) -> WorkflowResult:
        """
        ...
        try:
            ...
            while current_node_id and node_execution_count < self.max_node_executions:
                ...
                if current_node_id in ['check_og_range', 'check_srm_range', 'check_all_metrics']:
                    iterations += 1
            ...
            optimization_summary = self._create_optimization_summary(
                context.original_recipe,
                context.recipe,
                all_changes,
                iterations
            )
            
            logger.info(f"Workflow completed. Iterations: {iterations}, Changes: {len(all_changes)}")
    ...
    def _create_optimization_summary(self, original_recipe: Dict[str, Any], 
        ...
        # Categorize changes
        ingredient_changes = [c for c in changes if c.get('type') == 'ingredient_modified']
        ingredients_added = [c for c in changes if c.get('type') == 'ingredient_added']
        ingredients_removed = [c for c in changes if c.get('type') == 'ingredient_removed']
        ...
        return {
            ...
            'metric_improvements': {
                metric: {
                    ...
                    'change': final_metrics.get(metric, 0) - original_metrics.get(metric, 0)
                }
        ...
        for node_id, node in self.nodes.items():
            ...
            if 'next_node' in config:
                ...
                if next_node and next_node not in self.nodes:
                    errors.append(f"Node '{node_id}' references unknown next_node '{next_node}'")
                ...
                for path_key in ['yes_path', 'no_path']:
                    if path_key in config:
                        ...
                        if path_node and path_node not in self.nodes:
                            errors.append(f"Node '{node_id}' references unknown {path_key} '{path_node}'")
            ...
            if config.get('type') == 'multi_decision':
                ...
                for condition in conditions:
                    ...
                    if path_node and path_node not in self.nodes:
                        errors.append(f"Node '{node_id}' references unknown path '{path_node}'")
