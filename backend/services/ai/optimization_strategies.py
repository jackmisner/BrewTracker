...
class OptimizationStrategy:
    ...
    def _find_ingredients_by_name_contains(self, name_parts: List[str]) -> List[Dict[str, Any]]:
    ...
    def _calculate_target_from_style(self, metric: str, offset_percentage: float = 0) -> float:
    ...
    def execute(self, parameters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if target_malts_found:
            ...
            for grain in target_malts_found:
                ...
                new_amount = self._round_to_brewing_increment(new_amount, grain.get('unit', 'lb'))
            ...
            for grain in base_malts:
                current_amount = grain.get('amount', 0)
                new_amount = current_amount * (1 + increase_percentage * 0.5)  # Smaller increase for all
                new_amount = self._round_to_brewing_increment(new_amount, grain.get('unit', 'lb'))
...
class BaseMaltReductionStrategy(OptimizationStrategy):
    ...
    def execute(self, parameters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        parameters = parameters or {}
        reduction_target = parameters.get('reduction_target', '25_percent_below_style_max')
        ...
        for caramel in caramel_malts:
            ...
            for old_name, new_name in swap_ratios.items():
                if old_name.lower() in name:
                    ...
                    color_map = {
                        'crystal 40': 40, 'crystal 60': 60, 'crystal 80': 80, 'crystal 120': 120,
                        'caramel 40': 40, 'caramel 60': 60, 'caramel 80': 80
                    }
...
class HopIBUAdjustmentStrategy(OptimizationStrategy):
    ...
    def execute(self, parameters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        bittering_hops = [h for h in hops if h.get('use') == 'boil' and h.get('time', 0) >= 30]
        ...
        if strategy_type == 'increase_time_then_amount':
            if current_ibu < target_ibu:
                changes.extend(self._increase_hop_ibu(highest_ibu_hop, target_ibu, max_time, amount_increment))
        
        elif strategy_type == 'reduce_amount_then_time':
            if current_ibu > target_ibu:
                changes.extend(self._reduce_hop_ibu(highest_ibu_hop, target_ibu, min_amount))
        
        return changes
    
    def _find_highest_ibu_hop(self, hops: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """Find the hop with highest IBU contribution."""
        if not hops:
            return None
    ...
    def _increase_hop_ibu(self, hop: Dict[str, Any], target_ibu: float, max_time: int, amount_increment: float) -> List[Dict[str, Any]]:
        ...
        if current_time < max_time:
            ...
            new_amount = self._round_to_brewing_increment(new_amount, hop.get('unit', 'oz'))
            changes.append({
                'type': 'ingredient_modified',
                'ingredient_name': hop.get('name'),
                'field': 'amount',
    ...
    def _reduce_hop_ibu(self, hop: Dict[str, Any], target_ibu: float, min_amount: float) -> List[Dict[str, Any]]:
...
class YeastSubstitutionStrategy(OptimizationStrategy):
    ...
    def execute(self, parameters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        new_yeast_name = self._find_suitable_yeast(target_percent, maintain_style_compliance)
    ...
    def _find_suitable_yeast(self, target_attenuation: float, maintain_style: bool) -> Optional[str]:
...
def get_strategy(strategy_name: str, context: 'RecipeContext') -> Optional[OptimizationStrategy]:
