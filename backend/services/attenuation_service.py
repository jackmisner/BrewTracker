...
class AttenuationService:
    ...
    def is_valid_attenuation(attenuation: float) -> bool:
        ...
        return (
            AttenuationService.MIN_ATTENUATION
            <= attenuation
    ...
    def calculate_confidence_score(data_point_count: int) -> float:
        ...
        progress = (
            data_point_count - AttenuationService.MIN_DATA_POINTS_FOR_CONFIDENCE
        ) / (
            AttenuationService.MAX_DATA_POINTS_FOR_CONFIDENCE
            - AttenuationService.MIN_DATA_POINTS_FOR_CONFIDENCE
        )
    ...
    def process_completed_brew_session(session: BrewSession) -> bool:
        ...
        if not actual_attenuation or not AttenuationService.is_valid_attenuation(
            actual_attenuation
        ):
            return False
    ...
    @staticmethod
    def update_yeast_attenuation_data(
        ingredient_id: str, actual_attenuation: float
    ) -> bool:
        ...
        try:
            ...
            if len(ingredient.actual_attenuation_data) > 100:
                ingredient.actual_attenuation_data = ingredient.actual_attenuation_data[
                    -100:
                ]

            # Recalculate average
            ingredient.actual_attenuation_average = round(
                sum(ingredient.actual_attenuation_data)
                / len(ingredient.actual_attenuation_data),
                1,
            )

            # Update count
            ingredient.actual_attenuation_count = len(
                ingredient.actual_attenuation_data
            )

            # Update confidence score
            ingredient.attenuation_confidence = (
                AttenuationService.calculate_confidence_score(
    ...
    def get_improved_attenuation_estimate(ingredient_id: str) -> Optional[float]:
        ...
        try:
            ...
            improved_estimate = (
                theoretical_attenuation * theoretical_weight
    ...
    def get_attenuation_analytics(ingredient_id: str) -> Optional[dict]:
        ...
        try:
            ...
            analytics = {
                ...
                "improved_estimate": AttenuationService.get_improved_attenuation_estimate(
                    ingredient_id
                ),
                "last_update": (
                    ingredient.last_attenuation_update.isoformat()
                    if ingredient.last_attenuation_update
                    else None
                ),
            }
            ...
            if (
                ingredient.actual_attenuation_data
                and len(ingredient.actual_attenuation_data) > 1
            ):
                ...
                analytics["std_deviation"] = round(
                    (
                        sum(
                            (x - ingredient.actual_attenuation_average) ** 2
                            for x in data
                        )
                        / len(data)
                    )
    ...
    def get_all_yeast_analytics() -> List[dict]:
        ...
        try:
            ...
            for ingredient in yeast_ingredients:
                analytics = AttenuationService.get_attenuation_analytics(
                    str(ingredient.id)
                )
                if analytics:
