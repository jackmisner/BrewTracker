...
const IngredientManager: React.FC = () => {
  ...
  const sortIngredients = useCallback(
    (ingredients: IngredientWithSearch[]): IngredientWithSearch[] => {
  ...
  useEffect(() => {
    const loadIngredients = async (): Promise<void> => {
      try {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: "Failed to load existing ingredients",
        });
      }
    };
    loadIngredients();
                ...
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      Min Temperature (°{unitSystem === "metric" ? "C" : "F"})
                    </label>
                    <input
                      type="number"
                      ...
                      placeholder={
                        unitSystem === "metric" ? "e.g., 15" : "e.g., 60"
                      }
                      className="form-input"
                    />
                  </div>
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      Max Temperature (°{unitSystem === "metric" ? "C" : "F"})
                    </label>
                    <input
                      type="number"
                      ...
                      max={unitSystem === "metric" ? "38" : "100"}
                      placeholder={
                        unitSystem === "metric" ? "e.g., 22" : "e.g., 72"
                      }
                      className="form-input"
                    />
                  </div>
                    ...
                    <div key={type} className="ingredient-type-section">
                      ...
                      {expandedSections[type] && (
                        <div className="ingredient-type-items">
                          {ingredients.map(
                            (ingredient: IngredientWithSearch) => (
                              <div
