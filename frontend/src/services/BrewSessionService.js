...
class BrewSessionService {
   ...
   */
  async fetchBrewSession(sessionId) {
    try {
   ...
   */
  async createBrewSession(sessionData) {
    try {
   ...
   */
  async updateBrewSession(sessionId, sessionData) {
    try {
   ...
   */
  async deleteBrewSession(sessionId) {
    try {
   ...
   */
  async getBrewSessionsForRecipe(recipeId, forceRefresh = false) {
   ...
   */
  async getBrewSessionSummary(recipeId, forceRefresh = false) {
    try {
   ...
   */
  async getBrewingStats(recipeId) {
    try {
   ...
   */
  async getFermentationData(sessionId) {
    try {
   ...
   */
  async addFermentationEntry(sessionId, entryData) {
    try {
   ...
   */
  async getFermentationStats(sessionId) {
    try {
   ...
   */
  processBrewSessionsData(sessions) {
    ...
    return sessions
      ...
      .sort((a, b) => {
        ...
        return b.brew_date.getTime() - a.brew_date.getTime();
      });
  }
   ...
   */
  processBrewSessionData(session) {
   ...
   */
  findMostRelevantSession(sessions) {
   ...
   */
  validateBrewSessionData(sessionData, isCreate = true) {
    const errors = [];
   ...
   */
  validateFermentationEntry(entryData) {
    const errors = [];
   ...
   */
  formatBrewSessionForApi(sessionData) {
    const formatted = {
    ...
    if (sessionData.mash_temp)
      formatted.mash_temp = parseFloat(sessionData.mash_temp);
    if (sessionData.actual_og)
      formatted.actual_og = parseFloat(sessionData.actual_og);
    if (sessionData.actual_fg)
      formatted.actual_fg = parseFloat(sessionData.actual_fg);
    if (sessionData.actual_abv)
      formatted.actual_abv = parseFloat(sessionData.actual_abv);
    if (sessionData.actual_efficiency)
      formatted.actual_efficiency = parseFloat(sessionData.actual_efficiency);
    ...
    if (sessionData.batch_rating)
      formatted.batch_rating = parseInt(sessionData.batch_rating);
  ...
  getSessionDisplayName(session) {
    ...
    return `Session #${session.session_id?.substring(0, 6) || "Unknown"}`;
  }
  ...
  calculateConsistency(sessions) {
    ...
    const ogValues = sessions.map((s) => s.actual_og).filter((v) => v);
    const fgValues = sessions.map((s) => s.actual_fg).filter((v) => v);
    const abvValues = sessions.map((s) => s.actual_abv).filter((v) => v);
  ...
  calculateTrends(sessions) {
    const sortedSessions = [...sessions].sort(
      (a, b) => a.brew_date.getTime() - b.brew_date.getTime()
    );

    if (sortedSessions.length < 2) return null;

    const abvValues = sortedSessions.map((s) => s.actual_abv).filter((v) => v);
  ...
  calculateSessionDuration(session) {
    ...
    const endDate =
      session.packaging_date || session.fermentation_end_date || new Date();
   ...
   */
  createBrewSessionError(message, originalError) {
   ...
   */
  async safeNavigateToSession(sessionId, navigate) {
