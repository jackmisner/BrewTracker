...
class BeerXMLService {
  ...
  async exportRecipe(recipeId) {
    try {
      ...
      throw new Error(`Failed to export recipe: ${error.message}`);
    }
  }
  ...
  async parseBeerXML(xmlContent) {
    try {
      ...
      throw new Error(`Failed to parse BeerXML: ${error.message}`);
    }
  }
   ...
   */
  async matchIngredients(ingredients) {
    try {
      const response = await ApiService.beerxml.matchIngredients({
        ingredients,
      });

      return response.data.matching_results;
    } catch (error) {
      console.error("Error matching ingredients:", error);
      throw new Error(`Failed to match ingredients: ${error.message}`);
    }
  }
   ...
   */
  async createIngredients(ingredientsData) {
    try {
      const response = await ApiService.beerxml.createIngredients({
        ingredients: ingredientsData,
      });

      return response.data.created_ingredients;
    } catch (error) {
   ...
   */
  downloadBeerXML(xmlContent, filename = "recipe.xml") {
    try {
   ...
   */
  async processImportFile(file, availableIngredients) {
    try {
    ...
    recipes.forEach((recipe) => {
      if (recipe.ingredients) {
        ...
        recipe.ingredients.forEach((ingredient) => {
          ...
          if (summary.ingredientsByType[type] !== undefined) {
