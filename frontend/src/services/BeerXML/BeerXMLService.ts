...
class BeerXMLService {
   ...
   * BeerXML files are always in metric per spec, backend returns metric
   */
  async parseBeerXML(xmlContent: string): Promise<BeerXMLRecipe[]> {
    try {
      ...
      // All recipes are in metric per BeerXML spec
  ...
  /**
   * Convert recipe to target unit system
   * Uses the dedicated conversion endpoint
   */
  async convertRecipeUnits(
    ...
    try {
      ...
      return {
        recipe: (response.data as any).recipe,
        warnings: (response.data as any).warnings || [],
      };
    } catch (error) {
      console.error("Recipe conversion error:", error);
      throw new Error(
        `Failed to convert recipe units: ${(error as Error).message}`
      );
    }
  }
