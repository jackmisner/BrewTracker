...
describe("ViewRecipe", () => {
  ...
  describe("Loading States", () => {
    it("displays loading state while fetching recipe", async () => {
      RecipeService.fetchRecipe.mockImplementation(() => scenarios.loading());
    ...
    it("displays loading state for brew sessions separately", async () => {
      ...
      BrewSessionService.getBrewSessionsForRecipe.mockImplementation(() =>
        scenarios.loading()
      );
      BrewSessionService.getBrewSessionSummary.mockImplementation(() =>
        scenarios.loading()
      );
      BrewSessionService.getBrewingStats.mockImplementation(() =>
        scenarios.loading()
      );
  ...
  describe("Error Handling", () => {
    it("displays error message when recipe fetch fails", async () => {
      ...
      RecipeService.fetchRecipe.mockRejectedValue(new Error(errorMessage));
    ...
    it("displays recipe not found message", async () => {
      RecipeService.fetchRecipe.mockResolvedValue(null);
    ...
    it("handles brew session fetch errors gracefully", async () => {
      ...
      RecipeService.fetchRecipe.mockResolvedValue(mockRecipe);
      BrewSessionService.getBrewSessionsForRecipe.mockResolvedValue([]);
      BrewSessionService.getBrewSessionSummary.mockResolvedValue({ total: 0 });
      BrewSessionService.getBrewingStats.mockResolvedValue(null);
  ...
  describe("Recipe Display", () => {
    ...
    it("handles recipes without optional fields", async () => {
      ...
      RecipeService.fetchRecipe.mockResolvedValue(basicRecipe);
  ...
  describe("Brew Sessions Section", () => {
    ...
    it("displays brew sessions summary and stats", async () => {
      ...
      BrewSessionService.getBrewSessionSummary.mockResolvedValue(mockSummary);
      BrewSessionService.getBrewingStats.mockResolvedValue(mockStats);
    ...
    it("displays individual brew sessions", async () => {
      ...
      BrewSessionService.getBrewSessionSummary.mockResolvedValue({ total: 1 });
      BrewSessionService.getBrewingStats.mockResolvedValue(null);
  ...
  describe("Accessibility", () => {
    ...
    it("has accessible table structure for ingredients", async () => {
      ...
      RecipeService.fetchRecipe.mockResolvedValue(mockRecipeWithIngredients);
