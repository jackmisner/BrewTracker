...
describe("FeatureRequest", () => {
  ...
  describe("Page Structure", () => {
    test("renders page header with title and subtitle", () => {
      ...
      expect(screen.getByText("Help us improve BrewTracker by suggesting new features and enhancements.")).toBeInTheDocument();
    });

    test("renders form card with title", () => {
      expect(screen.getByText("Feature Request Form")).toBeInTheDocument();
      const formCard = screen.getByText("Feature Request Form").closest(".card");
  ...
  describe("Form Fields", () => {
    test("renders all required form fields", () => {
      ...
      // Optional fields
      expect(screen.getByLabelText("Alternative Solutions")).toBeInTheDocument();
    ...
    test("has correct input types and attributes", () => {
      ...
      expect(titleInput).toHaveAttribute("placeholder", "Brief description of the feature you'd like");

      const descriptionTextarea = screen.getByLabelText("Feature Description *");
    ...
    test("category select has correct options", () => {
      ...
      const categoryOptions = options.filter(option => 
        option.closest("select") === categorySelect
      );
      
      expect(categoryOptions).toHaveLength(5);
      expect(categoryOptions[0]).toHaveTextContent("Functionality - New brewing features");
      expect(categoryOptions[1]).toHaveTextContent("UI/UX - Interface improvements");
      expect(categoryOptions[2]).toHaveTextContent("Performance - Speed and efficiency");
      expect(categoryOptions[3]).toHaveTextContent("Integration - External tools/formats");
    ...
    test("priority select has correct options", () => {
      ...
      const priorityOptions = options.filter(option => 
        option.closest("select") === prioritySelect
      );
      
      expect(priorityOptions).toHaveLength(3);
      expect(priorityOptions[0]).toHaveTextContent("Low - Nice to have");
      expect(priorityOptions[1]).toHaveTextContent("Medium - Would improve workflow");
      expect(priorityOptions[2]).toHaveTextContent("High - Important for brewing process");
    });

    test("submit button has correct initial state", () => {
      const submitButton = screen.getByRole("button", { name: "Submit Feature Request" });
  ...
  describe("Form Interaction", () => {
    ...
    test("updates all textarea fields correctly", async () => {
      ...
      const proposedSolutionTextarea = screen.getByLabelText("Proposed Solution *");
      const alternativesTextarea = screen.getByLabelText("Alternative Solutions");
  ...
  describe("Form Validation", () => {
    test("prevents submission when required fields are empty", async () => {
      ...
      expect(window.open).toHaveBeenCalledWith(
        expect.stringContaining("title=Feature%20Request%3A%20"),
        "_blank"
      );
    });
    ...
    test("allows submission when all required fields are filled", async () => {
      ...
      await userEvent.type(screen.getByLabelText("Feature Title *"), "Test Feature");
      await userEvent.type(screen.getByLabelText("Feature Description *"), "Test description");
      await userEvent.type(screen.getByLabelText("Use Case *"), "Test use case");
      await userEvent.type(screen.getByLabelText("Proposed Solution *"), "Test solution");
  ...
  describe("Form Submission", () => {
    beforeEach(async () => {
      // Fill in all required fields
      await userEvent.type(screen.getByLabelText("Feature Title *"), "Test Feature Title");
      await userEvent.type(screen.getByLabelText("Feature Description *"), "Test feature description");
      await userEvent.type(screen.getByLabelText("Use Case *"), "Test use case description");
      await userEvent.type(screen.getByLabelText("Proposed Solution *"), "Test proposed solution");
      await userEvent.type(screen.getByLabelText("Alternative Solutions"), "Test alternatives");
    ...
    test("creates GitHub URL with correct parameters", async () => {
      ...
      expect(githubUrl).toContain("title=Feature%20Request%3A%20Test%20Feature%20Title");
    ...
    test("shows success message during submission", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Redirecting to GitHub to create your feature request...")).toBeInTheDocument();
      });

      const alert = screen.getByText("Redirecting to GitHub to create your feature request...").closest(".alert");
    ...
    test("resets form after successful submission", async () => {
      ...
      expect(screen.queryByText("Redirecting to GitHub to create your feature request...")).not.toBeInTheDocument();
    });
    ...
    test("handles empty alternatives field correctly", async () => {
      ...
      expect(decodedUrl).toContain("## Alternative Solutions\nNo alternatives considered");
    });
  });
  ...
  describe("Error Handling", () => {
    test("handles submission errors gracefully", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Error creating feature request. Please try again.")).toBeInTheDocument();
      });

      const alert = screen.getByText("Error creating feature request. Please try again.").closest(".alert");
    ...
    test("recovers from error state on subsequent submission", async () => {
      ...
      expect(screen.queryByText("Error creating feature request. Please try again.")).not.toBeInTheDocument();
    });
  });

  describe("GitHub URL Generation", () => {
    test("properly encodes special characters in form data", async () => {
      await userEvent.type(screen.getByLabelText("Feature Title *"), "Feature with & special chars!");
      await userEvent.type(screen.getByLabelText("Feature Description *"), "Description with <html> & \"quotes\"");
      await userEvent.type(screen.getByLabelText("Use Case *"), "Use case with %20 encoding");
      await userEvent.type(screen.getByLabelText("Proposed Solution *"), "Solution with newlines\nand tabs\t");
    ...
    test("includes all form fields in GitHub issue body", async () => {
      await userEvent.type(screen.getByLabelText("Feature Title *"), "Test Title");
      await userEvent.type(screen.getByLabelText("Feature Description *"), "Test Description");
      await userEvent.type(screen.getByLabelText("Use Case *"), "Test Use Case");
      await userEvent.type(screen.getByLabelText("Proposed Solution *"), "Test Solution");
      await userEvent.type(screen.getByLabelText("Alternative Solutions"), "Test Alternatives");
      ...
      expect(decodedUrl).toContain("Requested via BrewTracker Feature Request Form");
    });
    ...
    test("generates correct labels for GitHub issue", async () => {
      ...
      await userEvent.type(screen.getByLabelText("Feature Description *"), "Test");
      await userEvent.type(screen.getByLabelText("Use Case *"), "Test");
      await userEvent.type(screen.getByLabelText("Proposed Solution *"), "Test");
      await userEvent.selectOptions(screen.getByLabelText("Category"), "integration");
      ...
      const githubUrl = (window.open as jest.Mock).mock.calls[0][0];
      expect(githubUrl).toContain("labels=feature-request,medium-priority,integration");
    });
  });
  ...
  describe("CSS Classes and Structure", () => {
    ...
    test("applies correct CSS classes to form elements", () => {
      expect(document.querySelector(".feature-request-form")).toBeInTheDocument();
    ...
    test("applies correct CSS classes to alert messages", async () => {
      ...
      await userEvent.type(screen.getByLabelText("Feature Title *"), "Test Alert");
      await userEvent.type(screen.getByLabelText("Feature Description *"), "Test Alert Desc");
      await userEvent.type(screen.getByLabelText("Use Case *"), "Test Alert Use");
      await userEvent.type(screen.getByLabelText("Proposed Solution *"), "Test Alert Sol");
    ...
    test("form help text has correct class", () => {
      ...
      expect(document.querySelector(".form-help")).toHaveTextContent("This information helps us understand your environment");
    });
  });
