...
describe("FermentableInput", () => {
  const mockGrains = [
    ...
    },
    {
      ...
      type: "grain" as const,
      color: 60,
      grain_type: "caramel_crystal" as const,
      description: "Medium caramel malt",
      potential: 1.034,
    },
  ];
  ...
  test("resets form after successful submission", async () => {
    ...
    await waitFor(() => {
      expect((screen.getByPlaceholderText("Color") as HTMLInputElement).value).toBe("");
    });
  });
  ...
  test("auto-fills color when ingredient is selected", async () => {
    ...
    await waitFor(() => {
      const colorInput = screen.getByPlaceholderText("Color");
      expect((colorInput as HTMLInputElement).value).toBe("60");
    });
  });
