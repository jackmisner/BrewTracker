...
describe("UnitContext", () => {
  ...
  describe("convertUnit", () => {
    ...
    describe("weight conversions", () => {
      it("converts kg to lb", () => {
        ...
        expect((result as HTMLInputElement).value).toBeCloseTo(2.20462, 5);
        expect(result.unit).toBe("lb");
      });

      it("converts lb to kg", () => {
        const result = unitUtils.convertUnit(2.20462, "lb", "kg");
        expect((result as HTMLInputElement).value).toBeCloseTo(1, 5);
      ...
      it("converts oz to g", () => {
        ...
        expect((result as HTMLInputElement).value).toBeCloseTo(28.3495, 5);
        expect(result.unit).toBe("g");
      });

      it("converts kg to g", () => {
        const result = unitUtils.convertUnit(1, "kg", "g");
        expect((result as HTMLInputElement).value).toBe(1000);
        expect(result.unit).toBe("g");
      });

      it("converts g to kg", () => {
        const result = unitUtils.convertUnit(1000, "g", "kg");
        expect((result as HTMLInputElement).value).toBe(1);
        expect(result.unit).toBe("kg");
      });

      it("converts lb to oz", () => {
        const result = unitUtils.convertUnit(1, "lb", "oz");
        expect((result as HTMLInputElement).value).toBe(16);
        expect(result.unit).toBe("oz");
      });
    ...
    describe("volume conversions", () => {
      it("converts gal to l", () => {
        ...
        expect((result as HTMLInputElement).value).toBeCloseTo(3.78541, 5);
    ...
    describe("temperature conversions", () => {
      ...
      it("converts c to f", () => {
        ...
        expect((result as HTMLInputElement).value).toBe(32);
        expect(result.unit).toBe("f");
      });

      it("converts 212°F to 100°C", () => {
        const result = unitUtils.convertUnit(212, "f", "c");
        expect((result as HTMLInputElement).value).toBe(100);
        expect(result.unit).toBe("c");
      });

      it("converts 100°C to 212°F", () => {
        const result = unitUtils.convertUnit(100, "c", "f");
        expect((result as HTMLInputElement).value).toBe(212);
        expect(result.unit).toBe("f");
      });
    });
    ...
    describe("edge cases", () => {
      it("returns same unit when from and to are identical", () => {
        ...
        expect((result as HTMLInputElement).value).toBe(5);
        expect(result.unit).toBe("kg");
      });
  ...
  describe("getCommonUnits", () => {
    ...
    it("returns empty array for unknown measurement type", async () => {
      ...
      const units = result.current.getCommonUnits("unknown" as any);
      expect(units).toEqual([]);
    });
  });
